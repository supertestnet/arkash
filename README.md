...
The protocol can be summarized thusly: create a secret, and ensure that anyone who knows that secret can use it to withdraw money from a server via LN (with the server's cooperation) or on L1 (without its cooperation). Then share two piece of info with your recipient: the secret, and the server they need to talk to to get the money via LN. Voila! This makes the payment asynchronous. The recipient needn't be online when the sender creates the payment, and the sender needn't be online when the recipient redeems it.
